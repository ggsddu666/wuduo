...
//// 必须有下面两个宏 才能使用BOOST_AUTO_TEST_CASE
...
BOOST_AUTO_TEST_CASE(testLogStreamBooleans) {
    wuduo::LogStream os;
    const wuduo::LogStream::Buffer &buf = os.buffer();
...
BOOST_AUTO_TEST_CASE(testLogStreamIntegers) {
    ...
    // 这边<< 0x64 调用的是convert，而不是convertHex
    os << 0 << " " << 123 << 'x' << 0x64;
...
BOOST_AUTO_TEST_CASE(testLogStreamIntegerLimits) {
    ...
    os << 2147483647;
    BOOST_CHECK_EQUAL(buf.toString(), std::string("-2147483647-21474836482147483647"));
    os.resetBuffer();
...
BOOST_AUTO_TEST_CASE(testLogStreamStrings)
{
    ...
    std::string chenshuo = "world";
    os << chenshuo;
    BOOST_CHECK_EQUAL(buf.toString(), std::string("Hello world"));
}
...
BOOST_AUTO_TEST_CASE(testLogStreamFmts)
{
    ...
    // Fmt构造函数把1格式化
    // os << Fmt 是调用了Logstream中的nline LogStream& operator<<(LogStream& s, const Fmt& fmt);
    // 只是单纯把Fmt的字符串数据append到os中
    os << wuduo::Fmt("%4d", 1);
    BOOST_CHECK_EQUAL(buf.toString(), std::string("   1"));
    os.resetBuffer();

    os << wuduo::Fmt("%4.2f", 1.2);
    BOOST_CHECK_EQUAL(buf.toString(), std::string("1.20"));
    os.resetBuffer();

    os << wuduo::Fmt("%4.2f", 1.2) << wuduo::Fmt("%4d", 43);
...
BOOST_AUTO_TEST_CASE(testLogStreamLong) {
    ...
    // Logstream的append函数（31行左右）可以看出
    // 当avail() > len时候才能读入
    // std::cout << buf.avail() << std::endl;
